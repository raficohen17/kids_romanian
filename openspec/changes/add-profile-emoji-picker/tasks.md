...
- [x] 1. Define `PROFILE_EMOJIS` constant in `index.html` with the 20 curated emojis
- [x] 2. Add CSS for the emoji-picker modal: `.emoji-picker-modal`, `.emoji-grid`, `.emoji-tile`, `.emoji-tile.selected`
- [x] 3. Write `openEmojiPicker({ current, onSelect, onCancel })` helper that renders the modal and wires events
...
- [x] 8. `openspec validate add-profile-emoji-picker --strict` passes
